...
function handleOnDelivery() {
  ...
  let check = false;
  for (let i = 0; i < data.length; i++) {
    for (let t = 0; t < DUMMY_API.length; t++) {
      for (let j = 0; j < DUMMY_API[t].cart.length; j++) {
        for (let k = 0; k < DUMMY_API[t].cart[j].product.length; k++) {
          if (DUMMY_API[t].cart[j].product[k].processed === true) {
            check = true;
          }
        }
      }
  ...
  for (let i = 0; i < data.length; i++) {
    for (let t = 0; t < DUMMY_API.length; t++) {
      for (let j = 0; j < DUMMY_API[t].cart.length; j++) {
        for (let k = 0; k < DUMMY_API[t].cart[j].product.length; k++) {
          if (data[i].ID === DUMMY_API[t].cart[j].product[k].id && check) {
            const cartItem = document.createElement('tr');
              ...
              cartItem.innerHTML = `
                          ...
                          <td class="quantity">
                              <div class="quantity-item">
                                  ...
                                  <span id="quantity">${DUMMY_API[t].cart[j].product[k].quantity}</span>
      ...
      for (let j = 0; j < DUMMY_API[t].cart.length; j++) {
        for (let k = 0; k < DUMMY_API[t].cart[j].product.length; k++) {
          if (data[i].ID === DUMMY_API[t].cart[j].product[k].id && check) {
            ...
            if (DUMMY_API[t].cart[j].product[k].processed === false) {
              cartItem.classList.add('info-item-container');
